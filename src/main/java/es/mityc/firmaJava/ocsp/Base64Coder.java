...
public final class Base64Coder {
	...
	public static byte[] decode(Object input) {
		...
		if (input instanceof String) {
			...
			throw new IllegalArgumentException(ConstantesOCSP.STR_INPUT_NOT_PROPERLY_PADDED);
		}
		
		int iLen = in.length;
		if (iLen % 4 != 0)
			throw new IllegalArgumentException(
					ConstantesOCSP.STR_LENGTH_OF_BASE_64
					);
		...
		while (ip < iLen) {
			...
			if (i0 > 127 || i1 > 127 || i2 > 127 || i3 > 127)
				throw new IllegalArgumentException(ConstantesOCSP.STR_ILLEGAL_CHARACTER_IN_BASE_64);
			int b0 = map2[i0];
			int b1 = map2[i1];
			int b2 = map2[i2];
			int b3 = map2[i3];
